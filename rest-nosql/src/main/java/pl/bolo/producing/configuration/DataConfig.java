...
//        movie.setOriginalTitle("Jakis film");
//        list.add(movie);
//
//        return list;
//    }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(pathToFile))) {
            ...
            while ((reader.read()) != -1) {
                ...
                if (!details[3].equals("\\N")) {
                    if (details[3].length() > 100) {
                        ...
                        movie.setSlug(Slug.convertToSlug(details[3].substring(0, 99)));
                    } else {
                        movie.setOriginalTitle(details[3]);
                        movie.setSlug(Slug.convertToSlug(details[3]));
                    }
                }
            ...
            log.write(String.format("Loading fail. Problem with record %d", actualRecord));
            log.write(e.getMessage());
        }
        finally {
            log.write("Finish loading");
            return moviesList;
        }
    }
}
